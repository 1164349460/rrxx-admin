    ...
    <el-aside :width="isCollapse?'64px':'200px'">
      ...
      <el-menu default-active="1" class="el-menu-vertical-demo" background-color="#5895D3" text-color="#fff" active-text-color="#ffd04b" unique-opened router :collapse="isCollapse" collapse-transition>
        <el-submenu index="2">
          ...
          <el-menu-item index="/admin/employee/male">员工列表</el-menu-item>
        </el-submenu>
        <el-submenu index="4">
          <template slot="title">
          ...
          <el-menu-item index="/admin/customerManage/distribution">已分配客户</el-menu-item>
        </el-submenu>
        <el-submenu index="5">
          <template slot="title">
          ...
          <el-menu-item index="/admin/callManage/callhistory">呼出记录管理</el-menu-item>
        </el-submenu>
      </el-menu>
    </el-aside>
      ...
      tagsList: [
        {
          ...
          name: "employeeList",
          component: (resolve) =>
            require(["@/views/employeeManage/employeeList"], resolve),
          meta: {
            pathName: "员工列表",
          },
        },
  ...
  methods: {
    ...
    handleQuit() {
      this.$confirm("您确定要退出系统吗?", "退出", {
        ...
        logout({ id: this.$storage.userMessage("userMessage").id }).then((res) => {
          if (res.code === 0) {
